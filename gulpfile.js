...
gulp.task('_get-api-ref-page', () => {
  const data = { "index": { 
    "title": "API Reference",
    "long-title": "API Reference v<ngio-cheatsheet src='/angular/cheatsheet.json' version-only>2</ngio-cheatsheet>"
  } };
...
gulp.task('_get-guide', () => {
  const data = {
    ...
    "cheatsheet": {
      "title": "Cheat Sheet",
      "long-title": "Cheat Sheet v<ngio-cheatsheet src='/angular/cheatsheet.json' version-only>2</ngio-cheatsheet>"
    },
    "glossary": {
      "title": "Glossary",
...
function _getNgIoJadeForDir(dir, _data, _skiplist) {
  ...
  Object.keys(data).forEach(fileNameNoExt => {
    ...
    let pageConfig = 'WARNING: THIS FILE IS AUTOGENERATED FROM ANGULAR.IO; DO NOT EDIT\n'
          + 'layout: angular\n'
          + `title: "${entry.title}"\n`;
    if (entry['long-title']) pageConfig = pageConfig + `long-title: ${entry['long-title']}\n`;
    const desc = (entry.description || entry.intro || '').replace(/"/g, '\\"');
    if (desc) pageConfig = pageConfig + `description: "${desc}"\n`;
    // TODO: drop this extra property and trigger off of layout == angular instead:
    pageConfig = pageConfig + `angular: true\n`;
